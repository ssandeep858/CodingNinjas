...
//        });
        ...
        for (int i = 1; i < intervals.length; i++) {
            if (previousInterval[1] >= intervals[i][0]) {
                ...
                //response.add(new ArrayList<>(Arrays.asList(previousInterval[0],previousInterval[1])));
                previousInterval = intervals[i];
            }
        }
        // jab overlap nahi hoga tabhi res me add krunga otherwise line 38 pr likhta ye 
        //response.add(new ArrayList<>(Arrays.asList(previousInterval[0],previousInterval[1])));
        res.add(previousInterval);
        // if res empty then will take space given in brackets
        //return (int[][]) response.toArray();
