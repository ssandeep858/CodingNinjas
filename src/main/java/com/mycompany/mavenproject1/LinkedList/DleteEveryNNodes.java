 ...
 *         with two integers, 'M,' and 'N.' Traverse the linked list such that
 *         you
 ...
 *
 *         Note : No need to print the list, it has already been taken care.
 *         Only return
 *         the new head to the list. You can return null in case where all nodes
 *         will be
 *         deleted. Detailed explanation ( Input/output format, Notes, Images )
 *         Constraints : 1 <= t <= 10^2 0 <= P <= 10^5 Where P is the size of
 *         the singly
 ...
 *         For the first query, we delete one node after every zero elements
 *         hence removing all the items of the list. Therefore, nothing got
 *         printed.
 *
 *         For the second query, we delete three nodes after every two nodes,
 *         resulting in the final list, 1 ->
 *         2 -> 6 -> 7.
 */
public class DleteEveryNNodes {
    ...
    public static Node<Integer> skipMdeleteNEnhanced(Node<Integer> head, int M, int N) {
